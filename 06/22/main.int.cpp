...
void buildList(const string &filename, LinkedList<int> &list);
void deleteManager(LinkedList<int> &list);
void searchManager(const LinkedList<int> &list);
void displayManager(const LinkedList<int> &list);
...
void deleteManager(LinkedList<int> &list)
{
  ...
  while (toupper(targetCode[0]) != 'Q')
  {
    ...
    if (toupper(targetCode[0]) != 'Q')
    {
      ...
      if (list.deleteNode(stoi(targetCode)))
...
void searchManager(const LinkedList<int> &list)
{
  ...
  while (toupper(targetCode[0]) != 'Q')
  {
    ...
    if (toupper(targetCode[0]) != 'Q')
    {
      // College target;
      // target.setCode(targetCode);
      if (list.searchList(stoi(targetCode), aCollege))
        // aCollege.vDisplay();
        cout << "aCollege.vDisplay - " << aCollege << endl;
      else
        cout << "College \"" << targetCode << "\" was not found in this list." << endl;
    }
  }
