...
int main()
{
    Student list[] = {{"tom", 2.5}, {"bob", 3.2}, {"boc", 3.2}, {"mia", 3.9}, {"jack", 4.0}, {"john", 3.9}, {"zoe", 3.5}, {"ann", 3.1}, {"obb", 2.2}, {"bbo", 3.3}, {"ccm", 1.5}, {"", 0}};
    ...
    while (name != "Q")
    {
        ...
        if (hash.search(found, search, &key_to_index) != -1)
            cout << endl
                 << "Duplicate key: " << found.getName() << " - rejected! " << endl;
        else
        {
            cout << "Enter gpa ";
            cin >> gpa;
            search.setGpa(gpa);
            hash.insert(search, &key_to_index);
        }
        cout << "Enter name: ";
        cin >> name;
        search.setName(name);
    ...
    while (name != "Q")
    {
        ...
        if (hash.remove(itemOut, key, key_to_index))
            cout << endl
                 << itemOut.getName() << " " << itemOut.getGpa() << " - deleted!" << endl;
    ...
    while (name != "Q")
    {
        ...
        if ((hash.search(found, key, key_to_index)) != -1)
            cout << endl
                 << "Found: " << found.getName() << " " << found.getGpa() << endl;
        else
            cout << endl
                 << name << " not found!" << endl;
        cout << "Enter name or Q to quit: ";
